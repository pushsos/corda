...
class PersistentNetworkMapService(services: ServiceHubInternal, minimumPlatformVersion: Int)
    ...
    override val nodeRegistrations: MutableMap<PartyAndCertificate<Party>, NodeRegistrationInfo> = synchronizedMap(object : AbstractJDBCHashMap<PartyAndCertificate<Party>, NodeRegistrationInfo, Table>(Table, loadOnInit = true) {
        ...
        override fun keyFromRow(row: ResultRow): PartyAndCertificate<Party> = PartyAndCertificate(Party(X500Name(row[table.nodeParty.name]), row[table.nodeParty.owningKey]),
